...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['register'])) {
    ...
    if ($check->get_result()->num_rows > 0) {
        ...
        // Mã hóa mật khẩu và chèn vào DB với role là 'customer'
        ...
        if ($stmt->execute()) {
            ...
            $message = "<p style='color: #d9534f; background: #fdf7f7; padding: 10px; border: 1px solid #d9534f; margin-bottom: 20px; text-align: center;'>Lỗi hệ thống: " . $conn->error . "</p>";
        }
    }
}
?>
...
<main style="padding: 60px 0; background-color: #f9f9f9; min-height: 70vh; display: flex; align-items: center; justify-content: center;">
    <div style="background: #fff; padding: 40px; width: 100%; max-width: 500px; border: 1px solid #eee; box-shadow: 0 4px 15px rgba(0,0,0,0.05);">
        
        <h2 style="text-align: center; margin-bottom: 30px; letter-spacing: 2px; text-transform: uppercase;">Tạo Tài Khoản</h2>
        
        <?php if ($message) echo $message; ?>

        <form action="register.php" method="POST" id="form-register" style="display: flex; flex-direction: column; gap: 15px;">
            ...
            <button type="submit" name="register" style="background: #000; color: #fff; border: none; padding: 15px; font-family: 'Times New Roman', Times, serif; font-size: 16px; text-transform: uppercase; letter-spacing: 2px; cursor: pointer; margin-top: 10px; transition: 0.3s;">Đăng Ký</button>
        </form>
...
document.getElementById('form-register').addEventListener('submit', function(e) {
    ...
    // Kiểm tra rỗng
    if (fullname === '' || phone === '' || email === '' || username === '' || pass === '' || repass === '') {
        e.preventDefault();
