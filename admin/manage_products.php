...
// JOIN với bảng categories để lấy tên danh mục thay vì chỉ lấy ID
$products = $conn->query("SELECT p.*, c.name as category_name FROM products p LEFT JOIN categories c ON p.category_id = c.id WHERE p.status != 'deleted' ORDER BY p.id DESC");
?>
...
<div style="background: #fff; padding: 20px; border: 1px solid #ddd; margin-bottom: 30px;">
    ...
    <form action="manage_products.php" method="POST" enctype="multipart/form-data" id="form-add-product" style="display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 15px;">
        ...
        <div>
            ...
            <input type="text" name="unit" id="p_unit" placeholder="Cái, Chiếc, Bộ..." style="width: 100%; padding: 8px; border: 1px solid #ccc;">
        </div>

        <div>
            <label style="display: block; font-size: 12px; font-weight: bold; margin-bottom: 5px;">Số lượng ban đầu *</label>
            <input type="number" name="initial_quantity" id="p_qty" min="0" value="0" style="width: 100%; padding: 8px; border: 1px solid #ccc;">
        </div>
        <div>
            <label style="display: block; font-size: 12px; font-weight: bold; margin-bottom: 5px;">Giá bán đề xuất (VNĐ) *</label>
            <input type="number" name="suggested_price" id="p_price" min="0" style="width: 100%; padding: 8px; border: 1px solid #ccc;">
        </div>
        <div>
            <label style="display: block; font-size: 12px; font-weight: bold; margin-bottom: 5px;">Tỉ lệ lợi nhuận (%) *</label>
            <input type="number" step="0.1" name="profit_margin" id="p_margin" min="0" placeholder="VD: 20" style="width: 100%; padding: 8px; border: 1px solid #ccc;">
        </div>

        <div style="grid-column: span 2;">
            <label style="display: block; font-size: 12px; font-weight: bold; margin-bottom: 5px;">Mô tả chi tiết</label>
            <textarea name="description" rows="3" style="width: 100%; padding: 8px; border: 1px solid #ccc; resize: vertical;"></textarea>
        </div>
        <div>
            <label style="display: block; font-size: 12px; font-weight: bold; margin-bottom: 5px;">Hình ảnh đại diện</label>
            <input type="file" name="image" accept="image/*" style="width: 100%; padding: 5px; border: 1px solid #ccc;">
            
            <label style="display: block; font-size: 12px; font-weight: bold; margin-top: 10px; margin-bottom: 5px;">Trạng thái</label>
        ...
        <div style="grid-column: span 3;">
            ...
            <button type="submit" name="add_product" style="background: #000; color: #fff; border: none; padding: 10px 20px; font-family: 'Times New Roman', Times, serif; text-transform: uppercase; cursor: pointer;">Thêm Sản Phẩm</button>
        </div>
    </form>
</div>
            ...
            <?php if($products->num_rows > 0): ?>
                ...
                <tr>
                    <td style="text-align: center;">
                        <?php if($row['image']): ?>
                            <img src="../<?php echo $row['image']; ?>" alt="Ảnh" style="width: 50px; height: 50px; object-fit: cover; border: 1px solid #ddd;">
                        <?php else: ?>
                    ...
                    <td style="text-align: right; font-weight: bold; color: #d9534f;"><?php echo number_format($row['suggested_price'], 0, ',', '.'); ?>đ</td>
                    <td>
                        <?php if($row['status'] == 'visible') echo '<span style="color: #5cb85c; font-weight: bold;">Đang bán</span>'; else echo '<span style="color: #999;">Đang ẩn</span>'; ?>
                    </td>
                    <td>
                        <a href="edit_product.php?id=<?php echo $row['id']; ?>" style="color: #000; font-size: 12px; text-decoration: underline; margin-right: 10px;">Sửa</a>
                        <a href="delete_product.php?id=<?php echo $row['id']; ?>" style="color: #d9534f; font-size: 12px; text-decoration: underline;" onclick="return confirm('Bạn có chắc muốn xoá sản phẩm này?');">Xoá</a>
                    </td>
                </tr>
                <?php endwhile; ?>
            <?php else: ?>
                <tr><td colspan="8" style="text-align: center; padding: 20px;">Chưa có sản phẩm nào.</td></tr>
            <?php endif; ?>
        </tbody>
    </table>
</div>
